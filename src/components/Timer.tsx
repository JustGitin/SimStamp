...
export const Timer: React.FC<MyTimerProps> = (props) => {
  ...
  useEffect(() => {
    const fetchData = () => {
      const intervalId = setInterval(() => {
        ...
        setElapsedTime(now.diff(props.startStamp)); //falls undefined
      }, 500);
  ...
  function formatDuration(elapsedTime: Duration): string {
    return `${Math.floor(elapsedTime.as("hours"))}:${elapsedTime.toFormat(
      //hier nochmal in Luxon nachschauen, vermutlich nicht die beste Lösung auf diese weise
      "mm:ss"
    )}`;
  }

  return (
    <span className="durationOutput">Zeitspanne: {displayElapsedTime}</span>
  );
};
